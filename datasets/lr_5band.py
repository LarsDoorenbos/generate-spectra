...
from .sr_5band import process_spectrum as sr_process_spectrum
...
def process_spectrum(preprocessing, reduction, spectrum, redshift, wavelengths):
    ...
    subsampled_spectrum = normalized_spectrum[::8]
    
    return torch.as_tensor(subsampled_spectrum).float()
...
def training_dataset(params, output_path):
    ...
    temp_dataset = ImageSpectrumDatasetRS(train_dataset, lambda x: x, partial(sr_process_spectrum, 'none'), loglam, fromarray=False)
    ...
    # 10% + 512 for test & validation respectively
    ...
    train_dataset = ImageSpectrumDatasetRS(train_dataset, partial(transform_image, params["img_res"], train_means, train_stds), partial(process_spectrum, params["preprocessing"], params["reduction"]), loglam, fromarray=False)
    val_dataset = ImageSpectrumDatasetRS(val_dataset, partial(transform_image_eval, params["img_res"], train_means, train_stds), partial(process_spectrum, params["preprocessing"], params["reduction"]), loglam, fromarray=False)
...
def test_dataset(params, output_path):
    ...
    test_dataset = ImageSpectrumDatasetRS(test_dataset, partial(transform_image_eval, params["img_res"], train_means, train_stds), partial(process_spectrum, params["preprocessing"], params["reduction"]), loglam, fromarray=False)
